...
- **Deployment:** The Sanity.io project and Vercel production deployment are both live, set up manually by the user via their respective dashboards (no Vercel CLI in this environment). Sanity env vars are configured in Vercel → Project Settings → Environment Variables; Studio access follows the project-membership model described above under "Auth model."
